...
export class Peer extends ProxyNotifier<PeerEventTypes, PeerEvents> {
	...
	public send<E extends { type: string } & Record<string, any>>(payload: PeerEvents[PeerEventTypes] | E): Result<null, Error> {
		try {
	...
	public subscribe<T extends string, K extends Record<string, any>>(
		...
		return super.subscribe(type as any, listener as unknown as Listener<any>) as Subscription<PeerEvents[T]>;
	}
}
