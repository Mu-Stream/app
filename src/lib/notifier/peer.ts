...
export class Peer extends ProxyNotifier<PeerEventTypes, PeerEvents> {
  ...
  constructor({
    ...
    this._peer.once("signal", (singal) =>
      this._initial_signal.completeValue(singal),
    );
  ...
  public hookPluginEvents: Binder<string, any> = (
    key: string,
    handler: Listener<any>,
  ) => {
    // TODO: prevent proxying Core events
    this.proxy(
      key as PeerEventTypes,
      handler as unknown as Listener<PeerEvents[PeerEventTypes]>,
    );
    return Ok(null);
  };

  public send<K extends PeerEventTypes>(
  ...
  public subscribe<T extends PeerEventTypes>(
    ...
    return super.subscribe(
      type,
      listener as unknown as Listener<PeerEvents[T]>,
    ) as Subscription<PeerEvents[T]>;
  }
}
