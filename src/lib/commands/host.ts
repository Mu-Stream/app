...
export class HostCommand extends Command<CoreAppContext> {
	...
	private _registerPeer: WrappedListener<CoreAppContext, SignalingEvent['JOIN_OK']> = context => async payload => {
		...
		const new_peer_toast: PeerEvents['TOAST'] = { type: 'TOAST', severity: 'success', message: this._peer.username + ' a rejoint la salle' };
	...
	private _handleClose: WrappedListener<CoreAppContext, WithPeerIentity<PeerEvents['CLOSE']>> = context => async _ => {
		...
		const payload: PeerEvents['TOAST'] = { type: 'TOAST', severity: 'success', message: this._peer.username + ' a quitté la salle' };
		context.room.broadcast(payload);
		context.toaster.trigger(payload);
		return Ok(null);
	};

	private _handleToast: WrappedListener<CoreAppContext, WithPeerIentity<PeerEvents['TOAST']>> = context => async event => {
