...
export class HostCommand extends Command {
  ...
  private _registerPeer: WrappedListener<
    CoreAppContext,
    SignalingEvent["JOIN_OK"]
  > = (context) => async (payload) => {
    ...
    if ((await this._peer.link_done).isNone())
    ...
    this._peer.subscribe(
      "CURRENTLY_PLAYING",
      this._handleSongProgressPeer(context),
    );
  ...
  private _handlePeerStream: WrappedListener<
    CoreAppContext,
    PeerEvents["ADD_STREAM"]
  > = (context) => async (payload) => {
  ...
  private _handleSongProgressPeer: WrappedListener<
    CoreAppContext,
    PeerEvents["CURRENTLY_PLAYING"]
  > = (context) => async (payload) => {
    context.room.broadcast(payload, { excluded_ids: [this._peer.id] });
    return Ok(null);
  };
