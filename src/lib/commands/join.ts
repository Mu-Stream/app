...
import {
  Peer,
  type PeerEvents,
  type WithPeerIentity,
} from "$lib/notifier/peer";
...
export class JoinRoomCommand extends Command {
  ...
  async execute(context: CoreAppContext): Promise<Result<null, Error>> {
    ...
    const host_signal = (
      await context.signaling_server.once("SIGNAL_REQUESTER")
    ).unwrap();
  ...
  private _handleStream: WrappedListener<
    CoreAppContext,
    WithPeerIentity<PeerEvents["ADD_STREAM"]>
  ...
  private _handlePause: WrappedListener<
    CoreAppContext,
    WithPeerIentity<PeerEvents["PAUSE"]>
  > = (context) => async (_) => {
    context.audio_manager.pause();
    return Ok(null);
  };

  private _handleResume: WrappedListener<
    CoreAppContext,
    WithPeerIentity<PeerEvents["RESUME"]>
  > = (context) => async (_) => {
